...
var ttqw = {
    ...
    readNewsDetail(){
        ...
        if(newsItem){//可读新闻
            var result = util.boundsClick(newsItem);
            if(!result){
                return;
            }
            sleep(2000);
            var mr = util.clickByContainsText('展开全文');
            toast('展开全文 = '+mr);
    ...
    findOnClose(){
        var dialog_close = util.findDomById("iv_close")
        if(dialog_close){
            dialog_close.click()
        }
    },
    findNewsItem(){
        var newsItem = id("tv_from").findOnce(1);
        return newsItem;
    }
